...
let empty_agent handler error = 
 let error,interface = Int_storage.Quick_Nearly_inf_Imperatif.create handler error 0 in 
   error,{
    ...
    Cckappa_sig.agent_position = "",0,0;
         }
...
let translate_agent_sig parameters error handler agent kasim_id = 
              ...
              in
                Cckappa_sig.Site_map_and_set.add_map parameters 
                    ...
                    { 
                      ...
                      Cckappa_sig.site_position = port.Ckappa_sig.port_pos ;
 ...
 let error,c_interface = aux agent.Ckappa_sig.ag_intf error c_interface in  
    ...
    ({
      ...
      Cckappa_sig.agent_position = agent.Ckappa_sig.ag_pos 
            ...
            | [state] -> 
              ...
              let error,state_dic = 
                  Misc_sa.unsome 
                    ...
                    (fun error -> warn parameters error (Some "line 224") Exit (Cckappa_sig.Dictionary_of_States.init ()))
              in 
              let error,(bool,output) = Cckappa_sig.Dictionary_of_States.allocate_bool parameters error Misc_sa.compare_unit (Ckappa_sig.Internal state) () Misc_sa.const_unit state_dic in
              let error,internal = 
                match bool,output with
          ...
          | Ckappa_sig.LNK_VALUE (id_agent',agent',site',i,pos) ->
           ...
           | Ckappa_sig.LNK_TYPE (agent',site')  ->                          
              begin
               ...
               let error,site_name = 
                 match bool,output with
                  | _ , None  | true, _  -> warn parameters error (Some "line 328") Exit 0
                  | _ , Some (i,_,_,_) ->  
                 error,i
               in  
               let error,state_dic = 
                 Misc_sa.unsome 
                   (Int_storage.Nearly_Inf_Int_Int_storage_Imperatif_Imperatif.get parameters error (agent_name,site_name) handler.Cckappa_sig.states_dic)
                   (fun error -> warn parameters error (Some "line 367") Exit (Cckappa_sig.Dictionary_of_States.init ()))                             
               in 
               let error,(bool,output) = Ckappa_sig.Dictionary_of_agents.allocate_bool parameters error Misc_sa.compare_unit agent.Ckappa_sig.ag_nme () Misc_sa.const_unit handler.Cckappa_sig.agents_dic in  
               let error,agent_name' = 
                 match bool,output with
                   | _ , None  | true, _  -> warn parameters error (Some "line 349") Exit 0
                   | _ , Some (i,_,_,_) -> error,i
               in
               let error,site_dic' = 
                 match Int_storage.Nearly_inf_Imperatif.get parameters error agent_name' handler.Cckappa_sig.sites with 
               ...
               let error,site_name' = 
                 match bool,output with
                   | _ , None  | true, _  -> warn parameters error (Some "line 361") Exit 0
                   | _ , Some (i,_,_,_) ->  
                      error,i
               in 
               let state = Cckappa_sig.Lnk_type (agent_name',site_name') in  
...
let translate_mixture parameters error handler mixture = 
 ...
 let set_released_sites parameters error k ag ag' set = 
   Cckappa_sig.Site_map_and_set.fold2z_map parameters error  
 ...
 let translate_var parameters error handler var =
   match var with 
     | Ckappa_sig.VAR_KAPPA (x,y) ->
       let error,x = translate_mixture parameters error handler x in 
       error,Cckappa_sig.VAR_KAPPA(x,y)
     | Ckappa_sig.VAR_ALG(x,y) -> error,Cckappa_sig.VAR_ALG(x,y)
       
 let translate_obs parameters error handler obs = error,obs
   ...
   let error,c_variables =  
     List.fold_left 
       (fun (error,list) var -> 
         ...
         error,(var::list))
       (error,[]) compil.Ckappa_sig.variables 
   in 
   let error,c_signatures = 
     List.fold_left 
       (fun (error,list) (agent,pos) -> 
         let error,ag = 
           translate_agent_sig 
             parameters 
             error 
    ...
    let error,c_observables = 
     List.fold_left 
       (fun (error,list) obs -> 
         ...
         error,c_obs::list)
       (error,[]) compil.Ckappa_sig.observables  
    in 
     let error,c_inits = 
     List.fold_left 
       (fun (error,list) init -> 
   ...
   let error,c_perturbations = 
     List.fold_left 
       (fun (error,list) perturb -> 
         let error,c_perturb = translate_obs parameters error handler perturb in 
         error,c_perturb::list)
       (error,[]) compil.Ckappa_sig.perturbations  
   in 
