...
let main () =
    ...
    let compil =
        ...
        let _ =  List.iter (fun fic -> let _ = KappaLexer.compile fic in ()) files in 
			!Ast.result
    in 
    ...
    let refined_compil = Prepreprocess.translate_compil parameters_compil error compil in 
    ...
    let error = 
        if parameters_sig.Remanent_parameters_sig.trace or Print_handler.trace 
    ...
    let error = 
        if parameters_compil.Remanent_parameters_sig.trace or Print_cckappa.trace 
    ...
    let error = 
        if parameters_quark.Remanent_parameters_sig.trace or Print_quarks.trace 
    ...
    let error = 
      if parameters_influence_map.Remanent_parameters_sig.trace or Print_quarks.trace 
