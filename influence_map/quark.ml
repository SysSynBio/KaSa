  ...
  * Last modification: 2011, the 17th of March 
  * 
...
let empty_quarks parameter error handler = 
  ...
  let error,agent_var = Quark_type.AgentMap.create parameter error n_agents in 
  let error,site_var = Quark_type.SiteMap.create parameter error (n_agents,(0,0)) in 
  error,
    { 
      ...
      Quark_type.agent_var = agent_var ;
      Quark_type.site_var = site_var ;
    }
...
let add_agent parameters error rule_id agent_id agent_type = 
  let _ = Misc_sa.trace parameters (fun () -> (string_of_int rule_id)^(string_of_int agent_type)^"\n")
  in 
...
let add_site parameters error rule_id agent_id agent_type site_type state = 
  let _ = Misc_sa.trace parameters (fun () -> (string_of_int rule_id)^(string_of_int agent_type)^(string_of_int site_type)^(string_of_int state)^"\n")
  in 
...
let scan_var parameter error handler var_id var quarks = 
  match var 
  with 
    | Cckappa_sig.VAR_KAPPA (mixture,(string,pos)) -> 
      begin 
        let views = mixture.Cckappa_sig.views in 
        let agent_var =  quarks.Quark_type.agent_var in 
        let site_var = quarks.Quark_type.site_var in 
        let error,(agent_var,site_var) = (*what is tested in the mixture*)
             ...
             (fun parameter error agent_id agent (agent_test,site_test) -> 
                ...
                with 
                  ...
                  | Cckappa_sig.Agent agent -> 
                    ...
                    let error,agent_test = add_agent parameter error var_id kasim_id agent_type agent_test in 
                    let error,site_test = 
                         ...
                         (fun site port (error,site_test) -> 
                             ...
                             let rec aux k (error,site_test) = 
                               ...
                               then 
                                 ...
                                 aux (k+1) (add_site parameter error var_id kasim_id agent_type site k site_test) 
                             in 
        ...
        (agent_var,site_var)      
    in 
        error,{quarks with Quark_type.agent_var = agent_var ; Quark_type.site_var = site_var } 
        end  
    | Cckappa_sig.VAR_ALG (expr,(string,pos)) -> 
      begin
        error,quarks 
      end 
...
let scan_var_set parameter error handler vars quarks = 
  Int_storage.Nearly_inf_Imperatif.fold 
    parameter 
    error 
    (fun parameter error rule_id var quark_maps -> 
       let _ = Misc_sa.trace parameter (fun () -> "Var "^string_of_int rule_id^"\n") in 
        scan_var parameter error handler rule_id var quark_maps)
    vars
    quarks 
